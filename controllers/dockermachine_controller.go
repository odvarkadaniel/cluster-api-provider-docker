...
import (
	...
	"github.com/capi-samples/cluster-api-provider-docker/api/v1alpha1"
...
func (r *DockerMachineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	dockerCluster := &v1alpha1.DockerCluster{}
...
func patchDockerMachine(ctx context.Context, patchHelper *patch.Helper, dockerMachine *infrastructurev1alpha1.DockerMachine) error {
	// Always update the readyCondition by summarizing the state of other conditions.
	// A step counter is added to represent progress during the provisioning process (instead we are hiding the step counter during the deletion process).
	conditions.SetSummary(dockerMachine,
		conditions.WithConditions(
			infrastructurev1alpha1.ContainerProvisionedCondition,
...
func (r *DockerMachineReconciler) SetupWithManager(mgr ctrl.Manager) error {
	var (
		...
		controlledTypeGVK  = infrastructurev1alpha1.GroupVersion.WithKind(controlledTypeName)
	)
