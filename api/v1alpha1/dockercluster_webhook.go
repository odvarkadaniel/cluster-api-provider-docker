...
func (r *DockerCluster) Default() {
	...
	r.defaultDockerCluster()
}

func (r *DockerCluster) defaultDockerCluster() {
	if r.Spec.ControlPlaneEndpoint.Host == "" {
		r.Spec.ControlPlaneEndpoint.Host = "default-host-name"
	}
	if r.Spec.ControlPlaneEndpoint.Port == 0 {
		r.Spec.ControlPlaneEndpoint.Port = 8081
	}
}
...
func (r *DockerCluster) ValidateCreate() error {
	...
	if r.Name == "forbidden-docker-cluster-name" {
		return errors.New("forbidden docker cluster name is being used")
	}
